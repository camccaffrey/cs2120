...
def anti_reflexive := ∀ x, ¬ x ≺ x -- ex: less than
def irreflexive := anti_reflexive r -- sometimes used
def anti_symmetric := ∀ ⦃x y⦄, x ≺ y → y ≺ x → x = y -- ex: less than or equal
def asymmetric := ∀ ⦃x y⦄, x ≺ y → ¬ y ≺ x  -- ex: less than
...
example : ¬ reflexive r ↔ irreflexive r := _  -- true? FALSE, irreflexive is weaker/stronger than not reflexive???

-- transitive closure of a relation:
    -- the smallest relation such that r is transitive given certain values...
