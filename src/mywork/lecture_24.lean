...
reflexive? {} - Yes
What about this: {(0, 1), (2, 3)}                     - No, it's even irreflexive
...
def symmetric_closure := λ (a b : β), (r a b) ∨ (r b a)
  -- ^ this one could be wrong. might be ∧ or → instead of ∨  
...
-- definition that we added
def well_ordering := total_order r ∧
(
  ∀ (s : set β), -- for all sets of type set β 
  ∃ (b : β ),    -- there exists a b of type β
  (∀ (b' : β ), b' ∈ s → b ≺ b')
  )
/-
...
ctrl shift f    -- search for commands in lean?

strict (order) relation
 - asymmetric and transitive
 - NOT reflexive (you can derive this with a proof)
 - examples: less than, greater than

total (order) relation  
  - a.k.a. a strongly connected relation (I think)
  - given any two elements, they are related in at least one direction
  - total relation is reflexive, transitive, and antisymmetric becuase each element is related to every other in either direction exactly once
  - for natural numbers: less than or equal, greater than or equal

partial (order) relation
  - a relation that does not meet the first criterion for a total relation
...
what relation is this? less than or equal
