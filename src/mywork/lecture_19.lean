...
-- this is a for all proposition
begin
  show ∀ n, n = 0 ∨ n = 2 → n ∈ evens,  -- show: rewrite goal,
                                        -- so long as they are definitionally equal 
  assume n,
  assume zero_or_two,
  cases zero_or_two, -- cases when n = 0 or when n = 2
  rw zero_or_two, -- n = 0, n ∈ evens ->  0 ∈ evens

    -- case: n = 0
    unfold evens, -- plug in the definition of evens
    show { n | n % 2 = 0} 0, -- predicate that 0 is in the set of evens (true)
    show 0 % 2 = 0, -- predicate that 0 % 2 = 0 (true)

    exact rfl, -- reflexive property of equality

  -- case: n = 2
    cases zero_or_two,
/-
    show 2 ∈  evens, -- rewrites by definition are not necessary in Lean, so we can comment
    show evens 2,    -- this all out
    unfold evens,
    show 2 % 2 = 0,
-/
    exact rfl,
...
/- ------------------------------------------------
Tangent: the 4 color theorem
  - for any planar map, you just need four colors
------------------------------------------------ -/
